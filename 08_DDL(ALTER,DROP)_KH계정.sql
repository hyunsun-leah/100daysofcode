/*
    * DDL (DATE DEFINITION LANGUAGE)
    ...
    객체들을 생성(CREATE), TNWJD(ALTER), 삭제(DROP)하는 구문
...
--> 새로운 컬럼이 만들어지고 기본으로 NULL값으로 채워진다
...
-- 1_2)컬럼 수정(MODIFY)
--     데이터 타입 변경 시 : MODIFY 컬럼명 바꾸고자하는 데이터타입
--     기본값 변경 시     : MODIFY 컬럼명 DEFAULT 바꾸고자하는 기본값
SELECT * FROM DEPT_COPY;

--DEPT_ID 컬럼의 데이터 타입을 CHAR(3)으로 변경
ALTER TABLE DEPT_COPY MODIFY DEPT_ID CHAR(3);
-- ALTER TABLE DEPT_COPY MODIFY DEPT_ID NUMBER;
-- TABLE DEPT_COPY MODIFY DEPT_TITLE VARCHAR2(10); --> 변경하려는 크기를 초과하는 값이 없을 때만
...
--      데이터 값이 기록되어 있어도 같이 삭제됨! (단, 삭제된 컬럼 복구 불가능!!)
--      테이블에는 최소 한개의 컬럼은 존재해야됨!
